        ...
        <div className='flex justify-center items-center gap-5'>
          {
            tags.map((tag: string, index: number) =>
              <button onClick={() => setActiveTag(tag)} className={` capitalize font-semibold px-5 py-2 rounded-xl ${tag === activeTag ? "bg-green-500 text-gray-900" : "text-main"}`} key={index}>{tag == "yearly" ? <p className='flex items-center gap-2'>{tag} <span className='text-[12px] bg-white/10 rounded-xl px-4 py-1'>save 17%</span></p> : tag}</button>
            )
          }
        </div>
        <div className='grid grid-cols-1 md:grid-cols-3 md:gap-4 gap-7'>
          {
            pricing.map((item: TPricing) =>
              <div className={`bg-white/10 rounded-xl p-4  ${item.id === 2 ? "border border-header relative" : "border border-gray-700/50"} space-y-5`} key={item.id}>
                ...
                <button className={`border border-gray-700/50 w-full py-3 capitalize text-center text-main font-semibold rounded-xl ${item.id === 2 ? "bg-header hover:bg-header/90" : "hover:bg-header"}`}>
                  {item.button}
                </button>
